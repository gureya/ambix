...
void* iterate_array(void * args_v) {
    ...
    int c;
    while(!sig) {
        for (long i = args->start; i < args->size + args->start; i+=args->page_size / sizeof(int)) {
...
int main(int argc, char** argv) {
    ...
    long array_thread_size = array_size / n_threads;

    for(int i=0; i < n_threads; i++) {
        targs_t args = {array, array_thread_size, array_thread_size * i, page_size};
