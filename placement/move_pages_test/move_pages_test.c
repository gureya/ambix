...
unsigned int pagesize; // page size (should be 4kB)
unsigned int page_count = 10; // number of pages of a process to move
...
int get_node_list() {
        ...
        max_node = numa_max_node(); // Optane nodes should be well known by number
...
int main(int argc, char **argv)
{
	...
	if (nr_nodes == -1) {
		printf("Mismatch between configured nodes and memory-rich nodes.\n");
		exit(1);
	}
