...
void *memcheck_placement(void *args) {
    ...
    float last_balance_bw = 0.0;
    int last_balance_n = BALANCE_MIN;
    float max_dramBW = 0;
    int bw_samples = 0;
        ...
        if (switch_act) {
            ...
            if (memdata_lmod == 0 || (memdata_lmod == prev_memdata_lmod)) {
                ...
                if (!check_memdata(md)) {
                    ...
                    float pmm_bw_rd;
                    if (PMM_MIXED) {
                        pmm_bw_wr = md->sys_pmmAppBW;
                    }
                    else {
                        pmm_bw = md->sys_pmmWrites;
                    }
                    float pmm_bw_tot = pmm_bw_wr + pmm_bw_rd;
                    float dram_bw_tot = md->sys_dramReads + md->sys_dramWrites;

                    if (pmm_bw_wr > NVRAM_BW_WR_THRESH) {
                        ...
                        if (dram_usage >= DRAM_TARGET) {
                            ...
                            if (switch_migrated > 0) {
                                printf("NVRAM->DRAM: Sent %d out of %d write-intensive pages.\n", switch_migrated, n_pages);
                            }
                        }
                        ...
                        if (bw_samples <= 2) {
                            max_dramBW = (max_dramBW * bw_samples + dram_bw_tot)/ (bw_samples+1);
                            bw_samples++;
                        }
                        else if (max_dramBW > DRAM_BW_THRESH) {
                            int balance_n;
                            if (last_balance_bw < (dram_bw_tot + pmm_bw_tot)) {
                               balance_n = fmin(last_balance_n + BALANCE_MIN, MAX_N_FIND);
                               pthread_mutex_lock(&placement_lock);
                               switch_migrated = send_find(BALANCE_MIN, BALANCE_MODE);
                               pthread_mutex_unlock(&placement_lock);
                               last_balance_n = fmax(switch_migrated, BALANCE_MIN);
                            }
                            else {
                                balance_n = ;
                            }
