...
static int pte_callback_mem(pte_t *ptep, unsigned long addr, unsigned long next,
    ...
    if (!pte_soft_dirty(*ptep) && (n_backup < (n_to_find - n_found))) {
            // Add to backup list
    ...
    *ptep = pte_clear_soft_dirty(old_pte); // unset soft-dirty bit
...
/*static int pte_callback_mem_bal(pte_t *ptep, unsigned long addr, unsigned long next,
    ...
    if (!pte_soft_dirty(*ptep) && pte_young(*ptep)) {
        found_addrs[n_found].addr = addr;
        found_addrs[n_found++].pid_retval = curr_pid;
        return 0;
    }
...
static int pte_callback_balance(pte_t *ptep, unsigned long addr, unsigned long next,
                        struct mm_walk *walk) {
